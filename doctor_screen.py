"""
doctor_screen.py — Doctor Panel (ZEM HMS primary entry)
=========================================================
QA-12 FIX: All business-logic functions moved inside run() as closures
            so they can never reference undefined globals.
QA-19 FIX: Emergency tokens are now read from the Supabase DB
            (emergency_tokens table) instead of an in-memory queue that
            is disconnected from the patient kiosk.
"""
...
def run():
    """Doctor panel — all logic and UI defined here, never at module level."""

    global window   # kept global so prev_pat() can destroy it from inside closures
    ...
    def next_pat():
        """
        QA-19 FIX: Read next pending emergency token from Supabase DB.
        This is visible to BOTH the patient kiosk and the doctor panel.
        """
        selected_item = tree.selection()
        if not selected_item:
